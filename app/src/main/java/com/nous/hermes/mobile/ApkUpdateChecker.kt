 ...
 * - 下载时使用 ghproxy 代理加速（多个镜像逐个尝试 + GitHub 直连兜底）
 *
 * GitHub API 端点：
...
object ApkUpdateChecker {
    ...
    fun checkUpdate(
        ...
        return try {
            ...
            Log.i(TAG, "[GitHub] Current: $currentVersion, Latest: $latestVersion (tag=$tagName, prerelease=${release.optBoolean("prerelease")})")
